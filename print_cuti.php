...
$stmt2 = $conn->prepare("SELECT cuti.id_cuti, cuti.idpeg, cuti.ditetapkan, cuti.tanggal, cuti.daritgl, cuti.sampaitgl, cuti.lamacuti, cuti.alasan, cuti.pembuat_surat, pegawai.nama 
                         FROM cuti
                         JOIN pegawai ON cuti.idpeg = pegawai.idpeg
                         WHERE cuti.id_cuti = ? LIMIT 1");
$stmt2->bind_param('s', $id);
$stmt2->execute();
$stmt2->bind_result($id_cuti, $idpeg, $ditetapkan, $tanggal, $daritgl, $sampaitgl, $lamacuti, $alasan, $pembuat_surat, $namaPegawai);
$stmt2->fetch();
$stmt2->close();

$darisampai = "$daritgl/$sampaitgl";
$lamacuti = "$lamacuti hari";
...
// Dari & Sampai
$pdf->Cell($labelWidth, 10, 'Dari/Sampai', 0, 0, 'L'); // Kolom label Tgl/Waktu
$pdf->Cell($tabWidth, 10, ':', 0, 0, 'L'); 
$pdf->Cell($valueWidth, 10, $darisampai, 0, 1, 'L'); // Kolom nilai Tgl/Waktu

// Nama
$pdf->Cell($labelWidth, 10, 'Lama Cuti:', 0, 0, 'L'); // Kolom label Nama
$pdf->Cell($tabWidth, 10, ':', 0, 0, 'L'); 
$pdf->Cell($valueWidth, 10, $lamacuti, 0, 1, 'L'); // Kolom nilai Nama
